...
import ReactFlow, {
  Background, Controls, MiniMap, Handle, Position, type Edge, type Node,
...
function GraphNodeCard({ data }: { data: GraphNode }) {
  ...
  const statusConfig = {
    finished: { color: 'bg-green-100', text: 'text-green-800', border: 'border-green-200' },
    live: { color: 'bg-red-100', text: 'text-red-800', border: 'border-red-200' },
    ready: { color: 'bg-yellow-100', text: 'text-yellow-800', border: 'border-yellow-200' },
    pending: { color: 'bg-gray-100', text: 'text-gray-800', border: 'border-gray-200' },
    empty: { color: 'bg-gray-100', text: 'text-gray-800', border: 'border-gray-200' },
  };
      ...
      <div className="p-4 border-b border-gray-100">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <div className={`${config.accent} w-8 h-8 rounded-lg flex items-center justify-center`}>
              <span className="text-white text-sm font-bold">
                {data.type === 'match' ? 'M' : data.type === 'aggregator' ? 'A' : 'S'}
              </span>
            </div>
            <div>
        ...
        <div className="px-4 py-3 border-b border-gray-100">
          <div className="flex items-center justify-between">
            ...
            <div className={`
              ...
              ${filled === cap ? 'bg-green-100 text-green-800' : 
                filled > 0 ? 'bg-yellow-100 text-yellow-800' : 
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  <div className={`
                    ...
                    ${slot.participantId ? 'bg-green-500 text-white' : 'bg-gray-400 text-white'}
                  `}>
                    {slot.index + 1}
                  </div>
                ...
                {slot.participantId ? (
                  <div className="space-y-2">
                    <div className="bg-green-50 border border-green-200 rounded p-2">
                      <div className="text-xs text-green-600 font-medium mb-1">Participant ID</div>
                    ...
                    {slot.sourceNodeId && (
                      <div className="bg-blue-50 border border-blue-200 rounded p-2">
                        <div className="text-xs text-blue-600 font-medium mb-1">Source Node</div>
...
export default function TournamentGraphView({ graph }: { graph: TournamentGraph }) {
    ...
    <div className="h-[80vh] w-full rounded-lg border border-gray-200 bg-gray-50 relative">
      ...
      >
        <MiniMap 
          ...
          nodeColor="#6b7280"
        />
        <Controls className="!bg-white !border !border-gray-200 !rounded-lg shadow-sm [&>button]:!bg-white [&>button]:!border-gray-200 [&>button]:!text-gray-700 hover:[&>button]:!bg-gray-50" />
      ...
      {sinkNodes.length > 0 && (
        <div className="absolute bottom-4 left-4 right-4 p-4 bg-white border border-gray-200 rounded-lg shadow-sm">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h3 className="text-sm font-semibold text-gray-800">Final Results</h3>
          </div>
          <div className="space-y-2">
            {sinkEdges.map(edge => {
              ...
              return (
                <div key={edge.id} className="flex items-center gap-3 p-2 bg-gray-50 rounded border border-gray-200">
                  <div className="text-gray-600 font-mono text-xs">
                    {fromNode.id.slice(0, 8)}...
                  </div>
                  <div className="text-gray-400">→</div>
                  <div className="text-purple-700 text-xs font-medium">
                    {sink.kind === 'podium' ? `Position ${sink.podiumPos}` : sink.kind}
                  </div>
                </div>
              );
            })}
